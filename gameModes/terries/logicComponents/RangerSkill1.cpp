...
RangerSkill1::RangerSkill1(Context* context, Urho3DPlayer* main) :
...
RangerSkill1::~RangerSkill1()
{
}

void RangerSkill1::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill1", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(RangerSkill1, HandleHudButt));
...
void RangerSkill1::HandleHudButt(StringHash eventType, VariantMap& eventData)
{
	...
	if (name == "ranger")
	{
		skillSprite_->SetTexture(main_->cache_->
				GetResource<Texture2D>("Textures/terriesHud/icons/ranger/S_Bow11.png"));
	...
	if (name == "skill1")
	{
		...
		if (main_->mySceneNode_->GetComponent<SpriteSheetPlayer>()->focusedSprite_ == "ranger")
		{
			...
			SubscribeToEvent(E_TOUCHEND, HANDLER(RangerSkill1, HandleTouchEnd));
		}
	}
}

void RangerSkill1::HandleTouchEnd(StringHash eventType, VariantMap& eventData)
{
...
void RangerSkill1::Update(float timeStep)
{
	if (!enabled_)
	{
		return;
	...
	if (attackElapsedTime_ >= attackInterval_)
	{
		...
		for (float x = -45.0f; x < 45.0f; x += 10.0f)
		{
			...
			Vector3 dest = pos + (arrow->GetDirection() * dist);

			arrow->AddComponent(new KinematicMoveTo(context_, main_, dest, arrowSpeed_), 0, LOCAL);

			SubscribeToEvent(arrow, E_NODECOLLISIONSTART, HANDLER(RangerSkill1, HandleNodeCollisionStart));
		}
...
void RangerSkill1::HandleNodeCollisionStart(StringHash eventType, VariantMap& eventData)
{
