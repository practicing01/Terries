...
WarriorSkill2::WarriorSkill2(Context* context, Urho3DPlayer* main) :
...
WarriorSkill2::~WarriorSkill2()
{
}

void WarriorSkill2::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill2", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(WarriorSkill2, HandleHudButt));
	SetUpdateEventMask(USE_UPDATE);
}

void WarriorSkill2::HandleHudButt(StringHash eventType, VariantMap& eventData)
{
	...
	if (name == "warrior")
	{
		skillSprite_->SetTexture(main_->cache_->
				GetResource<Texture2D>("Textures/terriesHud/icons/warrior/A_Armour03.png"));
	...
	if (name == "skill2")
	{
		...
		if (main_->mySceneNode_->GetComponent<SpriteSheetPlayer>()->focusedSprite_ == "warrior")
		{
			...
			node_->GetComponent<Speed>()->speed_ -= 2.0f;
			...
			emitterStartFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/terriesTurtle.xml"));
			...
			particleStartNode_->GetComponent<SoundSource3D>()->Play(main_->cache_->GetResource<Sound>("Sounds/166681__univ-lyon3__hamel-marie-2012-13-son3.ogg"));

		}
	}
}

void WarriorSkill2::Update(float timeStep)
{
	if (cooling_)
	{
