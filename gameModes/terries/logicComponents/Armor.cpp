...
void Armor::Start()
{
	SubscribeToEvent(E_GETCLIENTHEALTH, HANDLER(Armor, HandleGetArmor));
	SubscribeToEvent(E_SETISSERVER, HANDLER(Armor, HandleSetIsServer));
	SubscribeToEvent(E_MODIFYCLIENTHEALTH, HANDLER(Armor, HandleModifyArmor));
...
void Armor::ModifyArmor(int armorMod, int operation, bool sendToServer)
{
	if (operation == 0)
	{
		armor_ = armorMod;
/*
		float armorPercent = (float)armor_ / 100.0f;
		...
		UIElement* guiSprite = main_->mySceneNode_->GetComponent<HUD>()->hud_->GetChild(node_->GetName());
		...
		UIElement* armorBar = guiSprite->GetChild("armor", true);
		((Sprite*)armorBar)->SetScale(1.0f, armorPercent);*/
...
/*
	...
	/*VariantMap vm;
	vm[ClientArmorSet::P_NODE] = node_;
	vm[ClientArmorSet::P_HEALTH] = armor_;
