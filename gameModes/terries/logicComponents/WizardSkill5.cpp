...
WizardSkill5::WizardSkill5(Context* context, Urho3DPlayer* main) :
...
WizardSkill5::~WizardSkill5()
{
}

void WizardSkill5::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill5", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(WizardSkill5, HandleHudButt));
	SetUpdateEventMask(USE_UPDATE);
}

void WizardSkill5::HandleHudButt(StringHash eventType, VariantMap& eventData)
{
	...
	if (name == "wizard")
	{
		skillSprite_->SetTexture(main_->cache_->
				GetResource<Texture2D>("Textures/terriesHud/icons/wizard/S_Holy01.png"));
	...
	if (name == "skill5")
	{
		...
		if (main_->mySceneNode_->GetComponent<SpriteSheetPlayer>()->focusedSprite_ == "wizard")
		{
			...
			SubscribeToEvent(E_TOUCHEND, HANDLER(WizardSkill5, HandleTouchEnd));
		}
	}
}

void WizardSkill5::HandleTouchEnd(StringHash eventType, VariantMap& eventData)
{
	...
	if (raeResult_.body_)
	{
		...
		node_->GetScene()->GetComponent<PhysicsWorld>()->GetRigidBodies(rigidBodies_, Sphere(dest_, radius_), 2);
		...
		if (rigidBodies_.Size())
		{
			for (int x = 0; x < rigidBodies_.Size(); x++)
			{
				...
				if (noed != node_)
				{
					...
					{
						noed->AddComponent(new Enchanted(context_, main_, 10.0f), 0, LOCAL);
					}
				}
			}
		}
		...
		emitterStartFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/enchant.xml"));
		emitterStartFX_->SetViewMask(1);
		particleStartNode_->SetPosition(dest_);
		...
		particleStartNode_->GetComponent<SoundSource3D>()->Play(main_->cache_->GetResource<Sound>("Sounds/191569__hast__fx8.ogg"));
...
void WizardSkill5::Update(float timeStep)
{
	if (cooling_)
	{
