...
RogueSkill5::RogueSkill5(Context* context, Urho3DPlayer* main) :
...
RogueSkill5::~RogueSkill5()
{
}

void RogueSkill5::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill5", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(RogueSkill5, HandleHudButt));
...
void RogueSkill5::Update(float timeStep)
{
	/*if (!enabled_)
	{
		return;
