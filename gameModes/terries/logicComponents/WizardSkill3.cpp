...
WizardSkill3::WizardSkill3(Context* context, Urho3DPlayer* main) :
...
WizardSkill3::~WizardSkill3()
{
}

void WizardSkill3::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill3", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(WizardSkill3, HandleHudButt));
	//SetUpdateEventMask(USE_UPDATE);

	XMLFile* xmlFile = main_->cache_->GetResource<XMLFile>("Objects/terriesGravija.xml");
...
void WizardSkill3::HandleHudButt(StringHash eventType, VariantMap& eventData)
{
	...
	if (name == "wizard")
	{
		skillSprite_->SetTexture(main_->cache_->
				GetResource<Texture2D>("Textures/terriesHud/icons/wizard/S_Light03.png"));
	...
	if (name == "skill3")
	{
		...
		if (main_->mySceneNode_->GetComponent<SpriteSheetPlayer>()->focusedSprite_ == "wizard")
		{
			...
			SubscribeToEvent(E_TOUCHEND, HANDLER(WizardSkill3, HandleTouchEnd));
		}
	}
}

void WizardSkill3::HandleTouchEnd(StringHash eventType, VariantMap& eventData)
{
	...
	if (raeResult_.body_)
	{
		...
		SubscribeToEvent(dot, E_NODECOLLISION, HANDLER(WizardSkill3, HandleNodeCollision));
		...
		dot->GetComponent<SoundSource3D>()->Play(main_->cache_->GetResource<Sound>("Sounds/276338__n-audioman__randomize60.ogg"));
...
void WizardSkill3::Update(float timeStep)
{
	if (cooling_)
	{
...
void WizardSkill3::FixedUpdate(float timeStep)
{
...
void WizardSkill3::HandleNodeCollision(StringHash eventType, VariantMap& eventData)
{
	if (!canAttack_)
	{
		return;
	...
	{
		otherNode->GetComponent<Health>()->ModifyHealth(int(otherNode->GetComponent<Health>()->health_ * 0.75f), -1, false);
