...
WarriorSkill4::WarriorSkill4(Context* context, Urho3DPlayer* main) :
...
void WarriorSkill4::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill4", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(WarriorSkill4, HandleHudButt));
	SetUpdateEventMask(USE_FIXEDUPDATE);
}

void WarriorSkill4::HandleHudButt(StringHash eventType, VariantMap& eventData)
{
	...
	if (name == "warrior")
	{
		skillSprite_->SetTexture(main_->cache_->
				GetResource<Texture2D>("Textures/terriesHud/icons/warrior/A_Shoes05.png"));
	...
	if (name == "skill4")
	{
		...
		if (main_->mySceneNode_->GetComponent<SpriteSheetPlayer>()->focusedSprite_ == "warrior")
		{
			...
			SubscribeToEvent(E_TOUCHEND, HANDLER(WarriorSkill4, HandleTouchEnd));
		}
	}
}

void WarriorSkill4::FixedUpdate(float timeStep)
{
	if (processing_)
	{
		...
		Vector3 victoria = dir_ * node_->GetComponent<Speed>()->speed_ * node_->GetScale().y_;
		node_->GetComponent<RigidBody>()->ApplyImpulse(victoria - node_->GetComponent<RigidBody>()->GetLinearVelocity());
	}
}

void WarriorSkill4::HandleNodeCollisionStart(StringHash eventType, VariantMap& eventData)
{
...
void WarriorSkill4::HandleTouchEnd(StringHash eventType, VariantMap& eventData)
{
	...
	if (raeResult_.body_)
	{
		//if (!node_->GetComponent<Blind>()->blinded_)
		{
			...
			dir_ = dir;
			dir *= node_->GetComponent<Speed>()->speed_ * node_->GetScale().y_;
			...
			SubscribeToEvent(node_, E_NODECOLLISIONSTART, HANDLER(WarriorSkill4, HandleNodeCollisionStart));
			...
			emitterStartFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/terriesSprint.xml"));
			...
			particleStartNode_->GetComponent<SoundSource3D>()->SetGain(0.05f);
			particleStartNode_->GetComponent<SoundSource3D>()->Play(main_->cache_->GetResource<Sound>("Sounds/273477__n-audioman__fatalstrike.ogg"));
