...
void Health::ModifyHealth(int healthMod, int operation, bool sendToServer)
{
	if (operation == 0)
	{
		...
		float healthPercent = (float)health_ / 100.0f;
		...
		if (!main_->mySceneNode_->HasComponent<HUD>())
		{
			return;
		}
		...
		UIElement* healthBar = guiSprite->GetChild("health", true);
		((Sprite*)healthBar)->SetScale(1.0f, healthPercent);
