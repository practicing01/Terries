...
ClericSkill3::ClericSkill3(Context* context, Urho3DPlayer* main) :
...
ClericSkill3::~ClericSkill3()
{
}

void ClericSkill3::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill3", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(ClericSkill3, HandleHudButt));
	SetUpdateEventMask(USE_UPDATE);
}

void ClericSkill3::HandleHudButt(StringHash eventType, VariantMap& eventData)
{
	...
	if (name == "cleric")
	{
		skillSprite_->SetTexture(main_->cache_->
				GetResource<Texture2D>("Textures/terriesHud/icons/cleric/S_Holy03.png"));
	...
	if (name == "skill3")
	{
		...
		if (main_->mySceneNode_->GetComponent<SpriteSheetPlayer>()->focusedSprite_ == "cleric")
		{
			...
			emitterStartFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/terriesHeroHeal.xml"));
			...
			particleStartNode_->GetComponent<SoundSource3D>()->Play(main_->cache_->GetResource<Sound>("Sounds/28783__voktebef__8bit.ogg"));
...
void ClericSkill3::Update(float timeStep)
{
	if (cooling_)
	{
