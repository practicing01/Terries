...
RogueSkill2::RogueSkill2(Context* context, Urho3DPlayer* main) :
LogicComponent(context)
{
	...
	enabled_ = false;
	radius_ = 1.0f;
	processInterval_ = 1.0f;
	processElapsedTime_ = 1.0f;
}

RogueSkill2::~RogueSkill2()
{
}

void RogueSkill2::Start()
{
	skillSprite_ = (Sprite*)(main_->mySceneNode_->GetComponent<HUD>()->
			hud_->GetChild("skill2", false)->GetChild(0)->GetChild(0));
	SubscribeToEvent(E_HUDBUTT, HANDLER(RogueSkill2, HandleHudButt));
	SetUpdateEventMask(USE_UPDATE);
}

void RogueSkill2::HandleHudButt(StringHash eventType, VariantMap& eventData)
{
	...
	if (name == "rogue")
	{
		skillSprite_->SetTexture(main_->cache_->
				GetResource<Texture2D>("Textures/terriesHud/icons/rogue/S_Shadow14.png"));
	...
	if (name == "skill2")
	{
		...
		if (main_->mySceneNode_->GetComponent<SpriteSheetPlayer>()->focusedSprite_ == "rogue")
		{
			if (!enabled_)
			{
				...
				emitterStartFX_->SetEffect(main_->cache_->GetResource<ParticleEffect>("Particle/terriesStunCloud.xml"));
				...
				particleStartNode_->GetComponent<SoundSource3D>()->Play(main_->cache_->GetResource<Sound>("Sounds/274209__littlerobotsoundfactory__whoosh-electric-02.ogg"));
...
void RogueSkill2::Update(float timeStep)
{
	if (!enabled_)
	{
		return;
	...
	processElapsedTime_ += timeStep;

	if (processElapsedTime_ >= processInterval_)
	{
		processElapsedTime_ = 0.0f;
	}
	else
	{
		return;
			...
			if (noed != node_)
			{
				if (noed->GetVar("npcType").GetInt() == 1)//terry
				{
					if (noed->HasComponent<Health>())
					{
						...
						{
							noed->AddComponent(new Stunned(context_, main_, 1.0f), 0, LOCAL);
						}
					}
				}
			}
